...
void* emucxl_memmove(void* dest, const void* src, size_t size) {
	...
	return (void*)cdest; // Note (ivan): added this line.
}

data_t **dataAlloc(void)
{
	// Note (ivan): think it's supposed to be data_t* and not data_t
	// but won't change yet
	return (data_t**)malloc(sizeof(data_t)); // TODO: check if this is correct or not malloc(sizeof(data_t*));
}
