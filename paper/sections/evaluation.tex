...
\begin{figure*}[htb]
     ...
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/max_dram.png}
         \caption{Max: DRAM}
         \label{fig-max_dram}
     \end{subfigure}\hfil
     \begin{subfigure}{0.23\textwidth}
         ...
         \caption{Max: CXL 400ns}
         \label{fig-max_low}
     \end{subfigure}\hfil
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/max_highlat_20250925233001_64_20.0_3-iter_hcxl0.png}
         \caption{Max: CXL 2$\mu$s}
         \label{fig-max_high}
     \end{subfigure}\hfil
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/max_10us.png}
         \caption{Max: CXL 10$\mu$s}
         \label{fig-max_10us}
     \end{subfigure}
    \medskip
      \centering
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/min_dram.png}
         \caption{Min: DRAM}
         \label{fig-min_dram}
     \end{subfigure}\hfil
     \begin{subfigure}{0.23\textwidth}
         ...
         \caption{Min: CXL 400ns}
         \label{fig-min_low}
     \end{subfigure}\hfil
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/min_high_lat20250924010812_64_0.5_5-iter_hcxl0.png}
         \caption{Min: CXL 2$\mu$s}
         \label{fig-min_high}
     \end{subfigure}\hfil
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/min_10us.png}
         \caption{Min: CXL 10$\mu$s}
         \label{fig-min_10us}
     \end{subfigure}
     \caption{Lock throughput (total iterations) across DRAM baseline and CXL latency configurations under max contention (top) and min contention (bottom) with cached access (Cached-HC for hardware locks, Cached-SC for software locks). Thread count ranges from 1 to 64.}
...
We evaluate each of the 13 locks under the applicable access modes at four latency points (DRAM, 400\,ns, 2\,$\mu$s, 10\,$\mu$s) across three contention levels (max, moderate, min).
This yields a total of $13 \times \{1\text{--}4\} \times 4 \times 3 = $ over 500 unique configurations, each repeated 5 times.
...
\paragraph{CXL 10\,$\mu$s (Figure~\ref{fig-max_10us}).}
At the highest tested latency, per-access costs dominate and algorithmic efficiency becomes the primary differentiator.
The relative ordering is preserved from lower latencies, but the absolute gap between lock families widens.
Elevator-BL-Array's $O(\log N)$ polling path limits the multiplicative cost of 10\,$\mu$s per access, while Peterson's $O(N^2)$ polling becomes prohibitive---its throughput drops to approximately 50K iterations at 32 threads.
Hardware locks (MCS, Ticket) retain reasonable performance because their polling paths hit local cache; the coherence overhead per CAS, while large in absolute terms, is amortized over many local polls.
...
\begin{figure*}[htb]
     ...
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/max_dram_cachedsc.png}
         \caption{Max: DRAM Cached-SC}
         \label{fig-max_dram_sc}
     \end{subfigure}\hfil
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/max_10us_cachedsc.png}
         \caption{Max: CXL 10$\mu$s Cached-SC}
         \label{fig-max_10us_sc}
     \end{subfigure}\hfil
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/min_dram_cachedsc.png}
         \caption{Min: DRAM Cached-SC}
         \label{fig-min_dram_sc}
     \end{subfigure}\hfil
     \begin{subfigure}{0.23\textwidth}
         \centering
         \includegraphics[width=\linewidth]{figures/min_10us_cachedsc.png}
         \caption{Min: CXL 10$\mu$s Cached-SC}
         \label{fig-min_10us_sc}
     \end{subfigure}
     \caption{Software lock throughput under Cached-SC access mode (with \texttt{clflushopt}/\texttt{sfence} after stores and \texttt{clflush}/\texttt{lfence} in polling loops) on DRAM and CXL 10\,$\mu$s. Compared to Figure~\ref{fig-cached-results}, flush overhead reduces throughput by 4--43$\times$ on DRAM, with contention-avoiding locks (Elevator-BL) least affected.}
     \label{fig-cachedsc-results}
\end{figure*}
...
Under moderate contention (1\,$\mu$s computation between acquisitions), the relative ordering is preserved but crossover points shift toward higher thread counts: at 400\,ns with 32 threads, MCS outperforms Elevator-BL (UC) by 28\% (vs.\ 9\% under max contention), as reduced contention decreases coherence invalidation frequency.
With a hash-table critical section (64-byte key, 256-byte value update), the access mode gap narrows further---from 9\% to 4\%---as critical section work dominates lock overhead.

\subsection{Discussion}
\label{sec:discussion}

\paragraph{Single-host vs.\ multi-host.}
Our single-host evaluation isolates coherence protocol overhead but does not capture cross-host effects (switch latency, inter-host snooping).
In multi-host deployments, we expect hardware coherence overhead to \emph{increase} as snoop filters must track ownership per host and back-invalidations must traverse the CXL switch.
Our results therefore represent a conservative lower bound on the advantage of avoiding hardware coherence; UC access would be largely unaffected.
...
Hardware lock throughput loss stems from (1)~per-CAS ownership round-trips and (2)~invalidation traffic from concurrent polling.
MCS mitigates source~(2) via per-thread polling but cannot avoid source~(1).
Software locks under UC avoid both, paying only the fixed CXL round-trip per access---explaining why the crossover thread count is lower for spin/ticket locks (both sources) than MCS (source~1 only).
Software locks require $O(N \log N)$ storage vs.\ $O(1)$ for spin/ticket, but at 64 threads the largest lock requires only 4.2\,KB.
