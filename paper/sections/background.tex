...
This section provides an overview of why shared FAM addresses datacenter challenges, the coherence models and memory access modes that CXL supports, and the mutex implementations that exploit them.

\subsection{CXL Fabric-Attached Memory for AI Workloads}

Distributed AI training is straining existing datacenter networks.
Large-scale systems struggle to fully utilize compute due to network overheads and bandwidth limitations in two-tier architectures where GPU clusters communicate over high-bandwidth NVLink while inter-node traffic traverses lower-bandwidth Ethernet~\cite{richinsAITaxHidden2019,tithiScalingIntelligenceDesigning2025,ultrascale_playbook,gangidiRDMAEthernetDistributed2024}.
This bandwidth asymmetry is an obstacle for emerging hybrid training-inference workloads~\cite{patelSplitwiseEfficientGenerative2024a}.

CXL fabric-attached memory offers a shared-memory programming model that reduces data movement overhead.
Multiple hosts transparently address the same memory pool without serialization or data copies, enabling collaborative processing at scale~\cite{tangYggdrasilReducingNetwork2024}.
Memory disaggregation via CXL addresses both memory underutilization through reduced overprovisioning and supports working sets that exceed single-server capacity~\cite{zhongMyCXLPool2025,liPondCXLBasedMemory2023,goukDirectAccessHighPerformance,kwonDisaggregatedMemorySystem2019,ewaisDisaggregatedMemoryDatacenter2023}.

Enabling this shared-memory model requires efficient inter-host synchronization.
Parallel communication primitives (AllReduce, AllGather, etc.) require thread-safe shared data structures, which in turn depend on scalable locks~\cite{dangAdvancedThreadSynchronization2017,balajiEfficientSupportMultithreaded2008,dozsaEnablingConcurrentMultithreaded2010}.
Non-scalable locks can cause dramatic performance collapse~\cite{boyd-wickizerNonscalableLocksAre}, making the choice of synchronization mechanism---and the coherence model it requires---a first-order architectural decision.
...
In modern multiprocessor systems, the operating system kernel monitors lock state and enables sleep/wake patterns for threads acquiring locks.
This notification mechanism is fundamentally inaccessible in a CXL FAM environment: there is no centralized monitor that can identify lock state across hosts and wake waiting threads.

We are therefore limited to polling: each thread repeatedly reads a memory location to detect value changes.
Both software and hardware mutexes rely on polling to learn whether they have acquired the lock.
This presents a notable performance challenge, as polling saturates both CPU resources and the CXL interconnect with read operations.
The impact of polling depends critically on the access mode:
under Cached-HC, repeated polls to an unmodified variable hit the local cache (zero CXL traffic);
...
When applicable, we implement exponential back-off to reduce redundant polling traffic across all access modes.
